...
def save_report_to_s3(report_id: str, transcript: str, report_text: str, source_file: str = None, model_id: str = None, prompt_name: str = None):
    ...
    if source_file:
        # Remove extension and add model name + prompt name + timestamp
        ...
        # Add timestamp + short unique ID to make unique across batch runs
        timestamp = datetime.utcnow().strftime('%Y%m%d-%H%M%S')
        prompt_suffix = f"-{prompt_name}" if prompt_name else ""
        file_key = f"{base_name}-{model_name}{prompt_suffix}-{timestamp}-{report_id[:8]}.json"
    else:
